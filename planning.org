...
** TODO org-git-insert-link-interacively: implement completion
   ...
   - branch names
   - Date?
** RESOLVED org-git-store-link interferes with other link storing mechanism
   :PROPERTIES:
   :ID:       6e7677d7-6f56-43fd-8c5b-c36d7db24706
   :END:
   With org-git-store-link at the beginning of
   org-store-link-functions, one cannot easily store links by ids
   to org entries, because org-id-store-link is never called. One
   could add org-git-store-link at the end of
   org-store-link-functions, or one could query the user wether a
   git link is wanted. Of course one could always customise
   everything in true org fashion.
** RESOLVED org-git-link messes up org-remember when remembering non-file buffers
   :PROPERTIES:
